...
DynamoRestore.prototype._sendBatch = function () {
    ...
    self.emit('send-batch', self.batches.length, self.requests.length, self.remainingLength)
}
...
DynamoRestore.prototype._cleanup = function (callback) {
    ...
    const params = {
        ...
        ProvisionedThroughput: {
            ReadCapacityUnits: self.options.schema.ProvisionedThroughput.ReadCapacityUnits,
            WriteCapacityUnits: self.options.schema.ProvisionedThroughput.WriteCapacityUnits
        }
    }

    self.emit('info', `Reverting back writeCapacity from ${self.options.concurrency} to ${params.ProvisionedThroughput.WriteCapacityUnits}`)
    return self.dynamodb.updateTable(params, callback)
}
